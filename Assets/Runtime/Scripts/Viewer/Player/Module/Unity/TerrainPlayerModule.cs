        ...
        public override void PlaySample(PlayerContext ctx, RawSample rawSample)
        {
            switch (rawSample.Payload)
            {
                ...
                case TerrainUpdate terrainUpdate:
                {
                    ...
                    if (terrainUpdate.HasEnabled) terrain.enabled = terrainUpdate.Enabled;
                    ...
                    if (terrainUpdate.HasTreeDistance) terrain.treeDistance = terrainUpdate.TreeDistance;
                    ...
                    if (terrainUpdate.HasBasemapDistance) terrain.basemapDistance = terrainUpdate.BasemapDistance;

                    if (terrainUpdate.HasLightmapIndex) terrain.lightmapIndex = terrainUpdate.LightmapIndex;
                    ...
                    if (terrainUpdate.HasDrawHeightmap) terrain.drawHeightmap = terrainUpdate.DrawHeightmap;

                    if (terrainUpdate.HasAllowAutoConnect) terrain.allowAutoConnect = terrainUpdate.AllowAutoConnect;

                    if (terrainUpdate.HasGroupingId) terrain.groupingID = terrainUpdate.GroupingId;

                    if (terrainUpdate.HasDrawInstanced) terrain.drawInstanced = terrainUpdate.DrawInstanced;
